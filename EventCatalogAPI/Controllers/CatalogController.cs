    ...
    public class CatalogController : ControllerBase
    {
                   ...
                   [FromBody] Event productToUpdate)
        {
            var catalogEvent = await _catalogContext.CatalogEvents
                              ...
                              (i => i.EventID == productToUpdate.EventID);
            if (catalogEvent == null)
            {
                return NotFound(new { Message = $"Item with id {productToUpdate.EventID} not found." });
            }
            catalogEvent = productToUpdate;
            _catalogContext.CatalogEvents.Update(catalogEvent);
            await _catalogContext.SaveChangesAsync();

            return CreatedAtAction(nameof(GetEventById), new { id = productToUpdate.EventID });
        }
        ...
        public async Task<IActionResult> DeleteProduct(int id)
        {
            var product = await _catalogContext.CatalogEvents
                .SingleOrDefaultAsync(p => p.EventID == id);
            if (product == null)
            {
                return NotFound();
